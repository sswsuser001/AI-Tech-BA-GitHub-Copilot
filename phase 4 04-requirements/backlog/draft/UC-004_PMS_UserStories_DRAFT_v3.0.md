...
# UC-004_PMS_UserStories_DRAFT_v3.0
...
- Notification behavior on proposal create is deferred
...
- Export performance for large result sets is limited by a 25 MB export boundary
...
- Partial failure behavior during bulk operations is deferred
...
- Conversion mapping to Invoice/Estimate fields is deferred

---

## Next Steps (for the author)

- Review each story with SMEs (Engineering, Finance, Product) to confirm field mappings, notification behavior, and lifecycle transition rules.
...
- This version reflects the confirmed business rules from the SME notes.
- Attachment types are limited to PDF, DOC, DOCX, and XLSX; file uploads over 50 MB are rejected.
- Notification behavior on proposal create is deferred and not included in this phase.
- Concurrency/locking is not required in this phase.
- Pagination is inherited from the existing proposal listing behavior.
- Export is limited to a 25 MB file size boundary.
- Exact lifecycle transition confirmation details are referenced in the folder-level rule file.
- Partial bulk failure handling and conversion mapping remain deferred for future follow-up stories.
